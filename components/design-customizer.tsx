...
export const BRANDS: BrandTokens[] = [
    ...
    logo: (
      <svg viewBox="0 0 640 86" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
        <g>
          ...
          <path d="M120.693 85.7021V0.0993091H178.194V17.4781H140.558V33.6651H176.197V50.2494H140.658V68.0254H180.39V85.7021H120.693Z" fill="#3f7fb6"/>
          <path d="M257.757 8.54042C261.251 5.16397 265.244 2.38337 269.736 0.0993091H185.781V17.776H209.939V85.7021H230.604V17.776H250.37C252.466 14.3995 254.962 11.321 257.757 8.54042Z" fill="#3f7fb6"/>
  ...
  },
  {
    ...
    accentGreen: '#ffa800',
    logo: (
      <img src="/brands/WildcasinoLogo.svg" alt="Neon Casino" className="h-6 w-auto" />
    ),
    logoSrc: '/brands/WildcasinoLogo.svg',
    lockupSrc: '/brands/neoncasino_lockup.svg',
    products: {
      sports: false,
      liveBetting: false,
      casino: true,
...
function loadActiveBrandId(): string {
  if (typeof window === 'undefined') return 'betheat'
  try {
    return localStorage.getItem(LS_BRAND_KEY) || 'betheat'
  } catch { return 'betheat' }
}
...
/** Swap logos in the DOM — finds default SVGs by viewBox and replaces */
function swapLogos(logoSrc: string | undefined, lockupSrc?: string, addVipEffect = false) {
  ...
  // Find all full logo SVGs (viewBox="0 0 640 86")
  ...
  // Find all "B" lockup SVGs (viewBox="0 0 114 86") and swap with brand lockup
  if (lockupSrc) {
...
export function DesignCustomizer({ onBrandChange, currentBrandId = 'betheat' }: DesignCustomizerProps) {
  ...
  useEffect(() => {
    ...
    if (savedBrandId && savedBrandId !== 'betheat' && !hasRestoredRef.current) {
  ...
  // MutationObserver: auto-swap any default SVG logos that appear after navigation
  useEffect(() => {
    const brand = BRANDS.find(b => b.id === activeBrandId)
    if (!brand || activeBrandId === 'betheat' || !brand.logoSrc) return
